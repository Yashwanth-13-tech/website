#!/usr/bin/env node
/**
 * BLR CRUIZ - Production Supabase PostgreSQL Safe Data Migration Utility
 * Migrates vehicles, locations, inquiries, bookings, and unavailability blocks
 * into Supabase PostgreSQL with idempotent schemas and row count verification.
 *
 * Usage:
 *   DATABASE_URL="postgresql://postgres.[ref]:[password]@aws-0-[region].pooler.supabase.com:6543/postgres" npm run migrate:supabase
 * or
...
const POSTGRES_URL = process.env.DATABASE_URL || process.env.POSTGRES_URL || process.env.SUPABASE_DB_URL

if (!POSTGRES_URL) {
  console.error('\n❌ ERROR: DATABASE_URL environment variable is required to run Supabase migration.')
  console.error('Example: DATABASE_URL="postgresql://postgres.xxx:password@aws-0-ap-south-1.pooler.supabase.com:6543/postgres" npm run migrate:supabase\n')
  process.exit(1)
}

const sqlitePath = path.resolve(__dirname, '..', 'data', 'blrcruiz.db')
const hasLocalSqlite = fs.existsSync(sqlitePath)

console.log('\n======================================================')
console.log('⚡ BLR CRUIZ: SUPABASE POSTGRESQL DATA MIGRATION')
console.log(`📁 Source SQLite: ${hasLocalSqlite ? sqlitePath : 'None (initializing Supabase schema only)'}`)
console.log(`🐘 Target Supabase DB: ${POSTGRES_URL.replace(/:[^:@]+@/, ':****@')}`)
console.log('======================================================\n')
...
async function runMigration() {
  ...
  try {
    // 1. Verify and create idempotent Supabase PostgreSQL tables
    console.log('1️⃣  Creating / Verifying Supabase PostgreSQL Schema...')
    await client.query(`
    ...
    let sourceVehiclesCount = 0
    let sourceLocationsCount = 0
    let sourceInquiriesCount = 0
    let sourceBookingsCount = 0

    if (hasLocalSqlite) {
      const sqliteDb = new DatabaseSync(sqlitePath)

      // 2. Migrate Vehicles
      console.log('2️⃣  Migrating Vehicles from SQLite...')
      try {
        const sqliteVehicles = sqliteDb.prepare('SELECT * FROM vehicles').all()
        sourceVehiclesCount = sqliteVehicles.length
        ...
        console.warn('   ⚠️ Vehicles migration warning:', err.message)
      }

      // 3. Migrate Locations
      console.log('3️⃣  Migrating Locations from SQLite...')
      try {
        const sqliteLocations = sqliteDb.prepare('SELECT * FROM locations').all()
        sourceLocationsCount = sqliteLocations.length
        ...
        console.warn('   ⚠️ Locations migration warning:', err.message)
      }

      // 4. Migrate Inquiries
      console.log('4️⃣  Migrating Inquiries from SQLite...')
      try {
        const sqliteInquiries = sqliteDb.prepare('SELECT * FROM inquiries').all()
        sourceInquiriesCount = sqliteInquiries.length
        ...
        console.warn('   ⚠️ Inquiries migration warning:', err.message)
      }

      // 5. Migrate Bookings
      console.log('5️⃣  Migrating Bookings from SQLite...')
      try {
        const sqliteBookings = sqliteDb.prepare('SELECT * FROM bookings').all()
        sourceBookingsCount = sqliteBookings.length
        ...
        console.warn('   ⚠️ Bookings migration warning:', err.message)
      }
    }

    // 6. Verify row counts in Supabase
    const [vCount, lCount, iCount, bCount] = await Promise.all([
      client.query('SELECT COUNT(*) FROM vehicles'),
      client.query('SELECT COUNT(*) FROM locations'),
      client.query('SELECT COUNT(*) FROM inquiries'),
      client.query('SELECT COUNT(*) FROM bookings'),
    ])

    const supabaseVehicles = parseInt(vCount.rows[0].count, 10)
    const supabaseLocations = parseInt(lCount.rows[0].count, 10)
    const supabaseInquiries = parseInt(iCount.rows[0].count, 10)
    const supabaseBookings = parseInt(bCount.rows[0].count, 10)

    console.log('\n======================================================')
    console.log('🎉 SUPABASE POSTGRESQL MIGRATION & VERIFICATION COMPLETE!')
    console.log('======================================================')
    console.log(`✔ Vehicles in Supabase:   ${supabaseVehicles} (Source SQLite: ${sourceVehiclesCount})`)
    console.log(`✔ Locations in Supabase:  ${supabaseLocations} (Source SQLite: ${sourceLocationsCount})`)
    console.log(`✔ Inquiries in Supabase:  ${supabaseInquiries} (Source SQLite: ${sourceInquiriesCount})`)
    console.log(`✔ Bookings in Supabase:   ${supabaseBookings} (Source SQLite: ${sourceBookingsCount})`)
    console.log('======================================================\n')
  } finally {
    client.release()
    await pgPool.end()
